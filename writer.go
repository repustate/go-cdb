...
// are the same present in mcdb.Writer so that can also satisfy this interface.
...
func (w *writer) Put(key, value []byte, tag uint8) error {
	// Compute new key with tag.
	taggedKey := append([]byte{byte(tag)}, key...)

	klen, dlen := uint32(len(taggedKey)), uint32(len(value))
	writeNums(w.wb, klen, dlen, w.buf)

	w.hash.Reset()

	// Write key (with tag) and update hash.
	_, err := w.hw.Write(taggedKey)
	if err != nil {
		return err
	}
